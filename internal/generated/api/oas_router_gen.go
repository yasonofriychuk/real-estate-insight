...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	default:
		...
		case '/': // Prefix: "/"
			...
			switch elem[0] {
			case 'd': // Prefix: "developments/search"

				if l := len("developments/search"); len(elem) >= l && elem[0:l] == "developments/search" {
					elem = elem[l:]
				} else {
					break
				}
			...
			case 's': // Prefix: "selection/"
				...
				if len(elem) == 0 {
					...
					switch r.Method {
					case "GET":
						s.handleSelectionByIdRequest([1]string{
							args[0],
						}, elemIsEscaped, w, r)
					default:
						s.notAllowed(w, r, "GET")
		...
		case '/': // Prefix: "/"
			...
			case 'd': // Prefix: "developments/search"
				...
				if len(elem) == 0 {
					...
					case "POST":
						...
						r.pathPattern = "/developments/search"
						r.args = args
						r.count = 0
						return r, true
					default:
			...
			case 's': // Prefix: "selection/"
				...
				if len(elem) == 0 {
					...
					switch method {
					case "GET":
						r.name = SelectionByIdOperation
						r.summary = "Get selection"
						r.operationID = "selectionById"
						r.pathPattern = "/selection/{selectionId}"
						r.args = args
						r.count = 1
						return r, true
					default:
